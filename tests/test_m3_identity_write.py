...
class TestM3IdentityWrite(unittest.TestCase):
    ...
    def test_verify_existing_mode_reads_and_validates(self):
        ...
        # Make the mock collection return some records
        mock_collection = self.create_legacy_stream()

        def collection_side_effect(path):
            if path == f"aziende/{REQUIRED_COMPANY_ID}/dipendenti" or path == f"aziende/{REQUIRED_COMPANY_ID}/utenti":
                return self.create_legacy_stream()
            return self.create_legacy_stream()
        ...
        self.assertIn("Post write validation failed", str(cm.exception))
        self.assertEqual(mig.status, "FAILED_POST_WRITE_VALIDATION")
        # Check that reports were still written!
        import os
        self.assertTrue(os.path.exists(os.path.join(self.args.output_dir, "M3_IDENTITY_POST_WRITE_VALIDATION.json")))
        self.assertTrue(os.path.exists(os.path.join(self.args.output_dir, "M3_POST_WRITE_FINGERPRINT_DIAGNOSTIC.json")))
