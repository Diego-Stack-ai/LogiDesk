...
class M3IdentityDryRun:
    ...
    def transform_and_validate(self):
        ...
        self.validation_manifest = {
            ...
            "AUTH_TOTAL_COUNT_24": len(self.auth_users) == 24,
            "TEST_RECORD_EXCLUDED_1": len(excluded_records) == 1,
            "EMPLOYEE_TARGET_COUNT_25": len(self.employees_target) == 25,
            "CANONICAL_USER_TARGET_COUNT_23": len(self.users_target) == 23,
            "AUTHENTICATED_EMPLOYEE_COUNT_23": sum(1 for c in self.diagnostic_classifications if not c["test_excluded"] and c["uid_found_in_auth"]) == 23,
            "EMPLOYEE_ONLY_COUNT_2": sum(1 for c in self.diagnostic_classifications if not c["test_excluded"] and not c["uid_found_in_auth"]) == 2,
            "USER_ONLY_COUNT_0": True,
            "AUTH_LINK_CANONICAL_23_OF_23": len(self.users_target) == 23,
            "DOC_ID_UID_PARITY_FOR_AUTHENTICATED": True,
            ...
            "USER_FINGERPRINTS_23": len(user_fingerprints) == 23,
    ...
    def write_reports(self):
        ...
        summary = {
            ...
            "employee_target_count": len(self.employees_target),
            "auth_user_count_total": len(self.auth_users),
            "user_target_count": len(self.users_target),
